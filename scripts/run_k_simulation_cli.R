...
if (file.exists("README.md") && file.exists("R") && file.exists("scripts")) {
  ...
  if (is.null(project_root) || !file.exists(project_root)) {
    ...
    cat("\nThen run: source('scripts/run_k_simulation_cli.R')\n")
...
main <- function() {
  ...
  tryCatch({
    source("R/simulation/print_k_simulation.R")
    cat("  - print_k_simulation.R loaded\n")
  }, error = function(e) {
    stop("Failed to load print_k_simulation.R: ", conditionMessage(e))
  })
