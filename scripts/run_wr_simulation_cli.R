# Run WR Simulation CLI - Generalized (WR-only)
#
# Usage (Rscript):
#   Rscript scripts/run_wr_simulation_cli.R --player="CeeDee Lamb" --season=2025 --week=17 --n_sims=1000
...
if (file.exists("README.md") && file.exists("R") && file.exists("scripts")) {
  ...
  if (is.null(project_root) || !file.exists(project_root)) {
    ...
    cat("\nThen run: source('scripts/run_wr_simulation_cli.R')\n")
...
main <- function() {
  ...
  tryCatch({
    source("R/simulation/print_wr_simulation.R")
    cat("  - print_wr_simulation.R loaded\n")
  }, error = function(e) {
    stop("Failed to load print_wr_simulation.R: ", conditionMessage(e))
  })
  ...
  if (!is.na(player_position) && player_position != "WR") {
