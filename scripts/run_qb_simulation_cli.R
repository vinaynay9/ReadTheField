# Run QB Simulation CLI - Generalized (QB-only)
#
# Usage (Rscript):
...
main <- function() {
  ...
  if (!is.na(target_player_id) && nzchar(trimws(target_player_id))) {
    ...
    if (exists("read_player_dim_cache")) {
      ...
      if (nrow(player_dim) > 0) {
        ...
        season_hits <- player_dim[player_dim$gsis_id %in% candidate_ids &
                                    ...
                                    player_dim$position == "QB", , drop = FALSE]
    ...
    if (nrow(partial_matches) > 1) {
      ...
      stop(
        "Ambiguous player name '", target_player_query, "'. QB matches found: ",
  ...
  if (!is.na(player_position) && player_position != "QB") {
