...
#' Build synthetic WR feature row for future games
#'
...
build_future_wr_feature_row <- function(player_id,
                                       season,
                                       week,
                                       team,
                                       opponent,
  ...
  log_msg("=== build_future_wr_feature_row ===")
  ...
  if (nrow(wr_features) == 0) {
    stop("WR weekly features cache is empty. Cannot build future feature row.")
  }

  historical_games <- wr_features[
    wr_features$player_id == player_id &
      wr_features$season == season &
      !is.na(wr_features$week) &
      wr_features$week < week,
    , drop = FALSE
  ]
