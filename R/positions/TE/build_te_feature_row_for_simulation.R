# Build TE Feature Row for Simulation (availability-aware)
#
# Constructs a single-row feature frame for simulation, respecting availability policy.

get_te_feature_groups <- function() {
...
has_meaningful_te_exposure <- function(row) {
...
resolve_schedule_row_te <- function(schedule, season, week, team) {
...
build_te_feature_row_for_simulation <- function(te_weekly_features,
                                                te_weekly_stats,
                                                player_dim,
  ...
  player_game_row <- te_weekly_features[
    te_weekly_features$player_id == player_id &
      te_weekly_features$season == season &
      te_weekly_features$week == week,
    , drop = FALSE
  ]
  ...
  if (nrow(player_game_row) > 0) {
    used_sources <- c(used_sources, "te_weekly_features")
  }

  exposure_row <- te_weekly_stats[
    te_weekly_stats$player_id == player_id &
      te_weekly_stats$season == season &
      te_weekly_stats$week == week,
    , drop = FALSE
  ]
  if (nrow(exposure_row) > 0) {
    ...
    used_sources <- c(used_sources, "te_weekly_stats")
  ...
  availability_state <- if (nrow(player_game_row) == 0) {
    "missing_row"
  } else if (has_meaningful_te_exposure(exposure_row)) {
    "observed_played"
  } else {
  ...
  if (policy == "played_only" &&
    ...
    reason <- if (availability_state == "missing_row") {
      "TE weekly features missing for player-week (bye/inactive/no stats row)."
    } else {
  ...
  if (nrow(te_weekly_features) == 0) {
    stop("te_weekly_features cache is empty. Cannot build counterfactual row.", call. = FALSE)
  }

  template <- te_weekly_features[0, , drop = FALSE]
  ...
  if (!is.null(te_weekly_stats) && nrow(te_weekly_stats) > 0) {
    seasons_played <- te_weekly_stats$season[te_weekly_stats$player_id == player_id]
