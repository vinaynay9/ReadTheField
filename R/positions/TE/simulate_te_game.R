# Simulate TE Game (v1 Contract)
#
# Monte Carlo simulation for TE player-game outcomes following TE v1 contract.

simulate_te_game <- function(feature_row, te_models, n_sims = 5000, availability_policy = "played_only") {
  ...
  if (is.null(feature_row) || nrow(feature_row) == 0) {
    stop("No feature row provided to simulate_te_game.")
  }
  ...
  regime <- determine_te_regime(week)
  valid_regimes <- get_te_regimes()
  if (!regime %in% valid_regimes) {
    stop("Invalid regime determined: ", regime)
  }

  if (is.null(te_models)) {
    stop("TE models are NULL. Cannot proceed with simulation.")
  }
  if (is.null(te_models$models)) {
    stop("TE models use legacy structure. Regime-based models required.")
  }
  models_list <- te_models$models
  ...
  if (is.null(prediction_regime)) {
    ...
    if (is.null(prediction_regime)) {
      stop("simulate_te_game could not find a valid regime for prediction.")
    }
  }

  missing_features <- setdiff(required_features, names(feature_row))
  if (length(missing_features) > 0) {
    stop("simulate_te_game missing required feature columns: ",
         paste(missing_features, collapse = ", "))
  }

  te_targets <- get_te_v1_targets()
  required_model_keys <- sapply(te_targets, function(t) get_te_model_key(t, prediction_regime))
  missing_models <- required_model_keys[!sapply(required_model_keys, function(k) !is.null(models_list[[k]]))]
  if (length(missing_models) > 0) {
    stop("Missing required TE v1 models for regime '", prediction_regime, "': ",
         paste(missing_models, collapse = ", "))
  }

  pred_data <- prepare_te_prediction_data(feature_row, week = week, required_features = required_features)

  get_baseline_value <- function(target_name, regime_name = NULL) {
    if (!is.null(regime_name)) {
      key <- get_te_model_key(target_name, regime_name)
  ...
  targets_model_key <- get_te_model_key("target_targets", prediction_regime)
  receptions_model_key <- get_te_model_key("target_receptions", prediction_regime)
  rec_yards_model_key <- get_te_model_key("target_rec_yards", prediction_regime)
  rec_tds_model_key <- get_te_model_key("target_rec_tds", prediction_regime)
  ...
  for (i in seq_len(n_sims)) {
    sim_targets[i] <- sample_from_model_te(
      targets_model,
      pred_data,
      n_samples = 1,
    ...
    sim_receptions[i] <- sample_from_model_te(
      receptions_model,
      pred_data,
      n_samples = 1,
      availability_policy = availability_policy,
    ...
    sim_rec_yards[i] <- sample_from_model_te(
      rec_yards_model,
      pred_data,
      n_samples = 1,
      availability_policy = availability_policy,
    ...
    sim_rec_tds[i] <- sample_from_model_te(
      rec_tds_model,
      pred_data,
      n_samples = 1,
      availability_policy = availability_policy,
...
prepare_te_prediction_data <- function(feature_row, week = NULL, required_features = NULL) {
  df <- as.data.frame(feature_row)

  if (!is.null(week) && exists("get_te_features_by_week")) {
    if (is.null(required_features)) {
      required_features <- get_te_features_by_week(week)
    }
...
predict_safe_te <- function(model, newdata, type = "response", n_samples = 1,
...
sample_from_model_te <- function(model, newdata, n_samples = 1, availability_policy = "played_only", fallback_mu = 0.5) {
  if (is.null(model)) {
  ...
  if (inherits(model_obj, "glm")) {
    ...
    if (!is.null(model_obj$transform) && model_obj$transform == "log1p") {
      sigma_val <- get_residual_sd_te(model_obj)
...
get_residual_sd_te <- function(model) {
  if (is.null(model)) {
...
compute_te_percentiles <- function(draws) {
