...
#' Determine TE v1 regime based on week number
determine_te_regime <- function(week) {
  case_when(
    week <= 3 ~ "early",
    week <= 5 ~ "mid",
    week <= 7 ~ "late",
...
#' Get canonical TE v1 feature contract by week (time-aware)
get_te_features_by_week <- function(week) {
...
#' Get canonical TE v1 feature contract by regime
get_te_features_by_regime <- function() {
  roll1_features <- c(
    "targets_roll1", "receptions_roll1", "rec_yards_roll1", "air_yards_roll1",
    "target_share_roll1", "air_yards_share_roll1",
    get_passing_defense_roll1_features()
...
validate_te_regime_features <- function(data_row, regime) {
  if (is.null(data_row) || nrow(data_row) == 0) {
    return(FALSE)
  }

  feature_contract <- get_te_features_by_regime()
...
get_te_model_key <- function(target, regime) {
  paste0(target, "__", regime)
}

#' Get all valid TE v1 regimes
get_te_regimes <- function() {
  names(get_te_features_by_regime())
}
