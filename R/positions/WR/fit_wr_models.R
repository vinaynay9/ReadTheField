# Fit WR Models (v1 Contract) - Robust with Fallbacks
#
# Fits statistical models for WR simulation following the WR v1 contract.
...
fit_wr_models <- function(training_data, min_rows = 200) {
  ...
  if (file.exists("R/positions/WR/wr_regime_v1.R")) {
    ...
    if (!exists("get_wr_regimes")) {
      stop("get_wr_regimes function not found. Regime system requires this function.")
    }
  ...
  if (is.null(training_data) || nrow(training_data) == 0) {
    stop("Empty training data provided. Cannot fit WR models.")
  }

  if (!"wr_regime" %in% names(training_data)) {
    stop("Missing wr_regime column in training data. Regime-based modeling requires wr_regime.")
  }

  if (exists("validate_wr_v1_target_schema")) {
    validate_wr_v1_target_schema(training_data, strict = TRUE)
  }

  if (any(is.na(training_data$wr_regime))) {
    training_data <- training_data[!is.na(training_data$wr_regime), , drop = FALSE]
  }

  valid_regimes <- get_wr_regimes()
  invalid_regimes <- setdiff(unique(training_data$wr_regime), valid_regimes)
  ...
  log_msg("=== WR Model Fitting Diagnostics ===")
  ...
  for (target in wr_targets) {
    for (regime in wr_regimes) {
      model_key <- get_wr_model_key(target, regime)
      log_msg("")
      log_msg("Fitting:", model_key)

      required_features <- feature_contracts[[regime]]
      if (is.null(required_features)) {
        stop("Unknown regime: ", regime, ". Valid regimes: ", paste(wr_regimes, collapse = ", "))
      }
