# Run WR Simulation - Pure Computation Layer
#
# Performs complete WR simulation workflow without any printing or file I/O.

run_wr_simulation <- function(gsis_id,
                              season,
                              week,
                              n_sims = 5000,
                              game_date = NULL,
                              ...
                              debug = FALSE) {
  if (missing(gsis_id)) {
    stop("run_wr_simulation requires gsis_id.", call. = FALSE)
  }
  ...
  wr_data_all <- read_wr_weekly_features_cache()
  if (nrow(wr_data_all) == 0) {
    stop("WR weekly features cache is empty.")
  }

  if (!is.null(identity_cache) && nrow(identity_cache) > 0) {
    orig_game_id <- wr_data_all$game_id
    orig_gameday <- wr_data_all$gameday
    ...
    if ("game_id_ident" %in% names(wr_data_all)) {
      wr_data_all$game_id <- ifelse(is.na(wr_data_all$game_id) | wr_data_all$game_id == "",
                                    wr_data_all$game_id_ident,
                                    wr_data_all$game_id)
    }
    if ("game_key_ident" %in% names(wr_data_all)) {
      wr_data_all$game_key <- ifelse(is.na(wr_data_all$game_key) | wr_data_all$game_key == "",
                                     wr_data_all$game_key_ident,
                                     wr_data_all$game_key)
    }
    if ("game_date_ident" %in% names(wr_data_all)) {
      wr_data_all$gameday <- ifelse(is.na(wr_data_all$gameday),
                                    wr_data_all$game_date_ident,
                                    wr_data_all$gameday)
    }

    helper_cols <- grep("(_ident)$", names(wr_data_all), value = TRUE)
    if (length(helper_cols) > 0) {
      wr_data_all <- wr_data_all[, setdiff(names(wr_data_all), helper_cols), drop = FALSE]
    }

    recovered_ids <- sum(is.na(orig_game_id) & !is.na(wr_data_all$game_id))
    recovered_dates <- sum(is.na(orig_gameday) & !is.na(wr_data_all$gameday))
    log_msg("Identity enrichment: recovered game_id for", recovered_ids, "rows; recovered gameday for", recovered_dates, "rows")
  }

  wr_data <- wr_data_all[wr_data_all$season %in% seasons_train, , drop = FALSE]
  if (nrow(wr_data) == 0) {
  ...
  wr_stats_all <- read_wr_weekly_stats_cache()
  if (nrow(wr_stats_all) == 0) {
    stop("WR weekly stats cache is empty.")
  }
  ...
  if (is_future) {
    ...
    if (!exists("build_wr_feature_row_for_simulation")) {
      stop("build_wr_feature_row_for_simulation not loaded.")
    }
    build_result <- build_wr_feature_row_for_simulation(
      wr_weekly_features = wr_data_all,
      wr_weekly_stats = wr_stats_all,
  ...
  if (!is.na(game_gameday)) {
    wr_data_pre <- wr_data_pre[is.na(wr_data_pre$gameday) | wr_data_pre$gameday < game_gameday, ]
  }
  if (nrow(wr_data_pre) == 0 || is.na(game_gameday)) {
    wr_data_pre <- wr_data[wr_data$season < game_season |
                             (wr_data$season == game_season & wr_data$week < game_week), ]
  }
  if (!is.na(game_season) && !is.na(game_week)) {
    wr_data_pre <- wr_data_pre[
      !(wr_data_pre$player_id == player_id &
          wr_data_pre$season == season &
          wr_data_pre$week == week),
      , drop = FALSE
    ]
  }
  if (!is.na(resolved_game_key)) {
    wr_data_pre <- wr_data_pre[is.na(wr_data_pre$game_key) | wr_data_pre$game_key != resolved_game_key, ]
  }
  if (!is.na(game_id)) {
  ...
  if (file.exists(stats_path) && requireNamespace("arrow", quietly = TRUE)) {
    ...
    if (!is.null(wr_stats) && nrow(wr_stats) > 0) {
      player_history <- wr_stats[
        wr_stats$player_id == player_id &
          (wr_stats$season < game_season |
             (wr_stats$season == game_season & wr_stats$week < game_week)),
        , drop = FALSE
      ]
  ...
  if (!exists("get_wr_features_by_week")) {
    stop("get_wr_features_by_week not available for time-aware feature selection.")
  }
  feature_cols <- get_wr_features_by_week(game_week)
  ...
  wr_models <- fit_wr_models(wr_data_pre, min_rows = 200)
  result$diagnostics$model_diagnostics <- wr_models$diagnostics

  trace_regime <- determine_wr_regime(game_week)
  ...
  for (tgt in wr_targets) {
    mk <- get_wr_model_key(tgt, trace_regime)
    diag_entry <- wr_models$diagnostics[[mk]]
    model_trace$targets[[tgt]] <- list(
      ...
      n_non_na = if (tgt %in% names(wr_data_pre)) sum(!is.na(wr_data_pre[[tgt]])) else NA_integer_,
  ...
  if (!"week" %in% names(player_feature_row) || is.na(player_feature_row$week)) {
    stop("run_wr_simulation failed to attach 'week' to feature_row.")
  }

  sim_result <- simulate_wr_game(
  ...
  if (exists("resolve_wr_simulation_schema")) {
    ...
    stop("resolve_wr_simulation_schema not found.")
  }
  ...
  if (exists("compute_wr_percentiles")) {
