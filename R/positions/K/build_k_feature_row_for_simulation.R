...
build_k_feature_row_for_simulation <- function(k_weekly_features,
  ...
  if (!is.null(player_game_row) && availability_state == "observed_played" && policy != "force_counterfactual") {
    ...
    has_season_history <- any(k_weekly_features$player_id == player_id &
                                k_weekly_features$season == season &
                                k_weekly_features$week < week)
    ...
    if (policy == "force_counterfactual" && !has_season_history) {
      history_any <- k_weekly_features[k_weekly_features$player_id == player_id, , drop = FALSE]
      if (nrow(history_any) > 0) {
        ...
        template <- k_weekly_features[0, , drop = FALSE]
  ...
  # Forced counterfactual: drop K recency roll features (player-level only).
  if (policy == "force_counterfactual" && availability_state != "observed_played") {
    roll_cols <- grep("_roll(1|3|5)$", names(feature_row), value = TRUE)
    roll_cols <- roll_cols[grepl("^target_fg_|^target_pat_", roll_cols)]
